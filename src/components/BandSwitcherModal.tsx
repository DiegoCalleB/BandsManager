...
import { Guitar, Check, Plus, Sparkles, X, Shield, ArrowRight, Loader2, Camera, Upload, Trash2 } from 'lucide-react';
...
export const BandSwitcherModal: React.FC<BandSwitcherModalProps> = ({
  ...
  const handleLeaveBand = async (bandId: string) => {
    if (!window.confirm("¿Estás seguro de que quieres abandonar esta banda? Perderás el acceso a sus datos.")) return;
    
    try {
      const response = await fetch(`/api/users/leave-band/${bandId}`, { 
        method: 'DELETE',
      ...
      if (response.ok) {
        ...
        alert("Error al abandonar la banda");
      }
    } catch (err) {
      console.error("Error al abandonar banda", err);
      alert("Error al abandonar la banda");
    }
  };
          ...
          {uniqueBands.map((band) => {
            ...
            return (
              ...
              >
                ...
                {!isActive && (
                  <button
                    onClick={(e) => { e.stopPropagation(); handleLeaveBand(band.band_id); }}
                    className="absolute top-3 right-3 z-20 p-1.5 rounded-xl bg-black/80 hover:bg-rose-600 text-rose-400 hover:text-white border border-rose-500/40 shadow-md transition-all cursor-pointer hover:scale-110 active:scale-95"
                    title="Abandonar esta Banda"
                  >
