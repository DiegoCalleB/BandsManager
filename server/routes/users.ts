...
router.delete('/leave-band/:bandId', requireAuth, async (req, res) => {
  try {
    ...
    if (state.userBands && Array.isArray(state.userBands)) {
      state.userBands = state.userBands.filter((ub: any) => {
        ...
        const isUserMatch = ub.user_id === user_id ||
          (userEmail && state.users?.some((u: any) => u.id === ub.user_id && u.email?.toLowerCase() === userEmail));
        return !(isUserMatch && ubClean === cleanTarget);
      });
    }

    // 2. Clear ownership/email on state.registeredBands if matching
    ...
    // 3. If user's active band_id is the band being left, update active band
    if (user) {
      const activeClean = user.band_id ? user.band_id.replace(/^(band|reg)-/, '') : '';
      if (activeClean === cleanTarget) {
        const remainingBands = buildAvailableBandsForUser(state, user);
        if (remainingBands.length > 0) {
          user.band_id = remainingBands[0].band_id;
          user.bandName = remainingBands[0].bandName || remainingBands[0].nombre_banda;
        } else {
    ...
    // 4. Delete from Supabase user_bands & registered_bands
...
router.delete("/users/:id", requireAuth, requireLeader, async (req, res) => {
  ...
  if (!otherBandsExist) {
    ...
    if (activeClean === cleanTarget) {
      ...
      if (remainingBands.length > 0) {
        targetUserObj.band_id = remainingBands[0].band_id;
        targetUserObj.bandName = remainingBands[0].bandName;
      } else {
